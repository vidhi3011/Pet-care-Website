...
<table class="fl-table" align="center">
   ...
   // $ids = join("','",$keys);
    ...
    foreach ($check as $x=>$value){
    $sql = "SELECT * FROM vet WHERE name='$x' ";
    $result = $con->query($sql);
    if($result){
    if ($result->num_rows > 0) {
        // output data of each row
        while($row = $result->fetch_assoc()) {
            ...
            echo "<tr><td>" . $row["name"].  "</td><td>". $row["phone"]. "</td><td>". $row["locate"].
            "</td><td><input  type='button' value = 'View' onclick='myFunction(\"".$row["name"]."\");'/></td></tr>";
        }}}}
...
function myFunction(name){
  ...
  window.location = "viewmap.php?name="+name;
}
</script>
<br><br><br><br>
